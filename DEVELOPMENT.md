...
## narou.rbとの連携方針（開発者向け）

- **コードの統合は行わない**: narou.rb のコードを取り込んだり、直接リンクする形の統合は行いません。
- **疎結合連携**: 外部プロセス呼び出し（CLI）／標準入出力の連携を前提にします。
- **拡張はCLIで**: 連携に必要な機能は、CLIオプションや出力フォーマットの拡張で対応してください（テンプレート側の調整も可）。
- **ライセンス順守**: 再配布時は、AozoraEpub3（GPL v3）および narou.rb（各プロジェクトのライセンス）を**同梱**してください。
...
- [AozoraEpub3 GitHub](https://github.com/AozoraJDK21-bot/AozoraEpub3-JDK21)
