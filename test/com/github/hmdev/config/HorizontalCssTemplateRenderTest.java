...
/**
 * Velocityテンプレート horizontal_text.vm に渡した値が
 * 期待どおりCSSへ反映されることの検証。
 */
public class HorizontalCssTemplateRenderTest {

    @Test
    public void horizontalTextCssReflectsValues() throws Exception {
        Path projectRoot = Paths.get(".").toAbsolutePath().normalize();
        if (!Files.exists(projectRoot.resolve("template"))) {
            Path testClasses = Paths.get(HorizontalCssTemplateRenderTest.class.getProtectionDomain().getCodeSource().getLocation().toURI());
        ...
        assertTrue(css.contains("font-size: 110%"));
        assertTrue(css.contains("line-height: 1.6"));
        assertTrue(css.contains("@page"));
        assertTrue(css.contains("1cm 1cm 2cm 3cm"));
        assertTrue(css.contains("4px 5px 6px 7px"));
        assertTrue(css.matches("(?s).*writing-mode:\\s*horizontal-tb.*"));
    }
}
