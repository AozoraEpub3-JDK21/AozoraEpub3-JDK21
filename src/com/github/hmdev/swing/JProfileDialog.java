...
public class JProfileDialog extends JDialog
{
	...
	public JProfileDialog(Image iconImage, String imageURLPath)
	{
		...
		try { jButtonCreate.setIcon(new ImageIcon(new URI(imageURLPath+"add.png").toURL())); } catch (Exception e1) { /* 意図的: アイコン読込失敗時はアイコン無しで起動 */ }
		...
		try { jButtonEdit.setIcon(new ImageIcon(new URI(imageURLPath+"edit.png").toURL())); } catch (Exception e1) { /* 意図的: アイコン読込失敗時はアイコン無しで起動 */ }
		...
		try { jButtonCancel.setIcon(new ImageIcon(new URI(imageURLPath+"cross.png").toURL())); } catch (Exception e1) { /* 意図的: アイコン読込失敗時はアイコン無しで起動 */ }
