...
- **仮名＋濁点/半濁点は対で判断する**: `※［＃半濁点付き平仮名か、1-4-87］` のように 2 文字に変換される字は、同梱の濁点フォント（`DakutenType=2`）や CSS 合成で表示できるので、対で出せるなら抑止しない。出せない場合も対ごと注記表示にするので `〓` と濁点が分かれることはない
...
- **表題ページの「横書き」が保存されない問題を修正**: GUI で横書きを選んでも ini の `TitlePage` が「中央」のままになっていました。同じ原因で「スタイル」タブの余白パネルが 1 つに合体していた表示崩れと、`@page` の余白が不正な CSS になる問題も修正
- **ダークテーマで「言語」「端末設定」等のメニュー文字が黒いままだった問題を修正**
- **表紙設定「表紙無し」が保存されない問題を修正**: 保存値を UI 言語に依存しない形式に変更したため、言語を切り替えても設定が保持されます
- **CLI の余白設定が不正な CSS になる問題を修正**: `presets/kobo_glo.ini` などを `-i` で渡すと `margin: 00 0.50 00 00` が出力されていました
...
# 更新確認（自動更新はしません。更新があればダウンロードページを案内します）
java -jar AozoraEpub3.jar --check-update
```

### 更新確認

GitHub の最新リリースと実行中のバージョンを比較して、更新の有無だけを知らせます。
**ダウンロードと差し替えは行いません**（自動更新なし・起動時の自動チェックもなし）。

- **GUI**: 画面上部の「更新確認」ボタン。更新があればダウンロードページを開くか確認します
- **CLI**: `java -jar AozoraEpub3.jar --check-update`（`-cu` でも可）

終了コードは、確認できれば `0`（更新の有無は問わない）、通信できなければ `1` です。
オフライン環境やプロキシ配下では失敗しますが、変換機能には影響しません。
...
| `1` | 1 つ以上の入力ファイルで変換に失敗した／`-i` の INI ファイル・`-d` の出力先ディレクトリ・入力ファイルが存在しない／オプションの指定が不正、または入力ファイルも `-url` も指定されていない／`--check-update` が通信できなかった |
