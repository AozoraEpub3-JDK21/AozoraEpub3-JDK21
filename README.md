...
- 本プロジェクトは **narou.rb での利用を前提** とした調整を行っていますが、プロジェクト間の**コード統合（組み込み・リンク）**は行いません。連携は、外部コマンド呼び出し等の疎結合な形を想定しています。
- **narou.rb は独立したプロジェクト**です。narou.rb の配布・変更は、それぞれのライセンスに従ってください。
- **AozoraEpub3 の再配布・改変には GPL v3** が適用されます。
- narou.rb から **AozoraEpub3 を CLI として実行**する利用形態は、GPL v3 における通常の使用に該当し、コードの結合やリンクには当たりません（ライセンス上の問題はありません）。
- 配布や同梱の可否・条件については、各プロジェクトのライセンスをご確認ください（当リポジトリでは個別の配布形態を推奨・保証しません）。
