...
- **「詳細設定」タブのスクロールが極端に遅い問題を修正**: マウスホイール 1 ノッチで 3 ピクセルしか動かなかった。1 ノッチで約 3 行分スクロールする（送り量は起動時のフォントメトリクスから算出）
...
- **GUI を FlatLaf でモダン化**: ライト / ダークテーマ同梱（既定はライト）。ini の `UiTheme` または画面右上のコンボで切替
- **ハーメルンの変換不能を修正**（話一覧 HTML 刷新への追従。旧キャッシュは初回のみ全話再取得）・**カクヨムの新着話取り逃しを修正**
- **GUI と CLI の設定既定値を統一**: ini にキーが無いときの CLI の挙動が GUI と一致。同梱 ini を使う CLI 変換は出力が変わります（詳細はリリースノート）
