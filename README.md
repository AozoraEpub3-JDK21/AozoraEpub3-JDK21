...
**Windows（ZIP）:**
```
AozoraEpub3-1.2.16-jdk21.zip
```

**Linux/macOS（TAR.GZ）:**
```
AozoraEpub3-1.2.16-jdk21.tar.gz
```
...
- **画像URL修正**: プロトコル相対URL（`//domain/path`）の画像（mitemin.net等）が404になる問題を修正
- **挿絵スキップ設定**: Webダウンロード設定に「挿絵スキップ」を追加。本文内の挿絵をダウンロードしない（表紙は除く）
