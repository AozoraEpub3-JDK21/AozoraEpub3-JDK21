...
| **0B-2-2** | `ArchiveCache`, `ArchiveScanner`, `ArchiveTextExtractor` | ★★ | 維持 | 全テスト + 比較 |
| **0B-2-3** | `GaijiInfo`, `ProfileInfo`, `ImageInfo`, `LatinConverter` | ★★ | 維持 | 全テスト + 比較 |
...
これにより `docs/stage-0b2-plan.md` §2.1 ★ファイル 8 個のうち、本 0B-2 (案 1) スコープで実際に Path 化対象となるのは 6 個 (`ImageUtils`, `GlyphConverter`, `AozoraGaijiConverter`, `Epub3ImageWriter` を 0B-2-1 で、残り `OutputNamer` / `JCoverImagePanel` は除外)。
