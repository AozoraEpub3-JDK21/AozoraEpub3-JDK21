...
| **0B-2-3** ⏳ | `ProfileInfo`, `ImageInfo`, `LatinConverter` (3 ファイル、`GaijiInfo` は §4.1.1 に分離) | ★★ | 維持 | 全テスト + 比較 |
| **0B-2-4** | `ImageInfoReader` (内部 Path-first 化、L419/L436 の `.toPath()` 重複解消含む) | ★★ | 維持 | 全テスト + 比較 |
| **0B-2-5** | `AozoraTextFinalizer`, `NarouFormatSettings` | ★★ | 維持 | 全テスト + 比較 |
| **0B-2-6** | `Epub3Writer`, `AozoraEpub3Converter` | ★★/★★★ | 維持 | 全テスト + 比較 |
| **0B-2-7** | `WebAozoraConverter` | ★★★ | 維持 | 全テスト + 比較 |
| **0B-2-8** | `JConfirmDialog`, `NarouFormatSettingsDialog`, `BookInfo` (内部のみ Path 補助化) | ★★ | 維持 | 全テスト + 比較 + 手動 GUI 確認 |
| **0B-2-9** | `AozoraEpub3`, `AozoraEpub3Applet` (両者で File occ ~114) | ★★★ | 維持 | 全テスト + 比較 + Codex レビュー + 手動 GUI/CLI 確認 |
...
これにより `docs/stage-0b2-plan.md` §2.1 ★/★★ファイルのうち、本 0B-2 (案 1) スコープで実際に Path 化対象となるのは: 0B-2-1 で 4 個 (`ImageUtils`, `GlyphConverter`, `AozoraGaijiConverter`, `Epub3ImageWriter`)、0B-2-2 で 3 個 (`ArchiveCache`, `ArchiveScanner`, `ArchiveTextExtractor`)、0B-2-3 で 3 個 (`ProfileInfo`, `ImageInfo`, `LatinConverter`)。除外 3 個 (`OutputNamer`, `JCoverImagePanel`, `GaijiInfo`) は案 2 / Phase B 行き。
