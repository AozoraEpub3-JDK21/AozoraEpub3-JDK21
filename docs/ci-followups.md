...
## 1. スタック PR で CI が起動しない — ✅ 対応済み（2026-08-09）
...
となっていたため、**ベースが別のフィーチャーブランチである PR では 1 つも起動しなかった**。
チェックが赤にも緑にもならず「未実施」のまま素通りする。

### 対応（2026-08-08 ユーザー承認 → 2026-08-09 実施）

両ワークフローの `pull_request.branches` を `'**'` にした。
...
- `ci.yml` — `[ main, master ]` → `[ '**' ]`。`paths-ignore`（`**.md` / `docs/**` ほか）はそのまま。
  **ドキュメントのみの PR では引き続き `CI` は起動しない**点に注意（`Tests` は起動する）
- `test.yml` — `[ master, develop, main ]` → `[ '**' ]`
- `push` トリガー（`ci.yml` の `[ main, master ]`）は変更していない。
  フィーチャーブランチへの push ごとに走らせると実行回数が跳ね上がるため、
  ブランチ上の検証は PR 経由に寄せる
...
`paths-ignore` に当たった場合や、PR を作らずに検証したい場合は明示実行する。
...
- 上記 1 の `'**'` 対応（2026-08-09 実施済み）により、子 PR にも CI が走るようになり
