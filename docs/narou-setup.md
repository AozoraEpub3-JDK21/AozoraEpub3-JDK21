...
GUI の「変換完了後に自動でプレビューを開く」にチェックを入れる（または `AozoraEpub3.ini` に `AutoPreview=1` を書く）と、narou.rb / narou.rs 経由の変換でも、変換が終わるたびに出来上がった EPUB がブラウザでプレビュー表示されます。プレビューは別プロセスで開くため narou.rb の処理は止まりません。ブラウザのタブを閉じればプレビュー用のプロセスも自動終了します。
