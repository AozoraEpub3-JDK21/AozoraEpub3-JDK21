...
- **static 状態が 2 つ増えている** (`PreviewLauncher.current` /
  ...
  借金であり、Phase 2 で棚の入口 (`--library <フォルダ>`) を足す際に肥大させないこと
...
- **本棚は別ページにせず、`.body` と入れ替えて出す** (重ねない)。重ねると裏に残った
  iframe が本文を読み込み続ける。`#libraryView` はヘッダとフッタの間に入る flex の兄弟
- **本棚ボタンを出すかどうかは `api/session` で決める。** そのために
  `libraryFolder` (フォルダ名) と `libraryCount` を足した。一覧まで載せると、
  本文を読むだけのセッション情報に最大 2000 件が付いて回る。
  一覧は本棚を開いた時と ⟳ を押した時にだけ `api/library` へ取りに行く (ポーリングしない)。
  `libraryCount` は 📚 ボタンの tooltip (「本棚 (l) — N 冊」) に出し、
  `api/library` を取り直すたびに更新する
- **本を切り替えている間は次の切り替えを受け付けない** (`libraryOpening`)。
  `state` は 1 冊ぶんしか無いため、連打すると 2 つの読み込みが同じ `state` を書き合い、
  遅い方が後に終わると `state.bookId` と表示中の本が食い違う。
  同じ理由で、インスペクタは**要求時の bookId を控えて、戻ってきたときに変わっていたら結果を捨てる**
- **本文が無い本 (spine 0 件) は `loadBook` でエラーにする。** 黙って受け入れると
  書名だけ新しくなって iframe には前の本が映ったまま残る
- **`l` キーは iframe の document にも登録する。** iframe 内の keydown は親へ伝播せず、
  本文をクリックするとページ送りのためにフォーカスが iframe へ移るため、
  登録しないと「読書中だけ `l` が効かない」ことになる
- **既定の本が無くても起動できる。** 棚だけを読み込んだ場合 (CLI の `--library <フォルダ>`)
  は本棚を開いた状態で始まり、本に対する操作 (目次・セクション送り・📂・ⓘ) は
  `setBookControlsEnabled(false)` で押せなくする。本棚の ✕ も戻る先が無いので出さない
- **本の切り替えはページを読み込み直さずに行う。** 読み込み直すと `pagehide` で
  `api/bye` が飛び、サーバが「ビューアーが閉じた」と判断しうる (猶予 20 秒はあるが、
  わざわざ危ない方を通らない)。代わりに書籍に紐づく `state` を明示的に戻す。
  **`el.frame` の `data-path` を消すのを忘れないこと** — 別の本でも 1 番目のセクションが
  `OPS/xhtml/title.xhtml` になることは多く、消さないと `gotoSection` が
  「同じセクション」と判断して前の本を映したままになる (実機で再現を確認済み)。
  開けなかったときは元の本の状態へ戻す (消された本を選ぶと 404 になる)
- **表紙 URL にキャッシュ避けを付けない。** C2 が `no-cache` + ETag にしてあるので、
  ブラウザは毎回問い合わせて 304 を受け取る。パラメータを足すと毎回作り直しになる
- **本を開いたら URL を `?book=<id>` に書き換える** (`history.replaceState`、履歴は増やさない)。
  再読み込みしても同じ本が開く。パスは今の URL のまま使うこと —
  `index.html` などに変えると相対 URL の解決が変わり、`api/` 配下へ届かなくなる
- **打ち切りは黙って行わない。** 一度に描くのは 200 枚まで (`LIBRARY_PAGE_SIZE`) とし、
  残りは「さらに表示 (残り N 冊)」と冊数を出したボタンで足す。
  絞り込み中は「一致した冊数」と「棚全体の冊数」を混ぜずに出す
- 並び順は更新日 (新しい順 / 古い順)・書名・著者の 4 通り。日本語の照合は
  `Intl.Collator('ja')`。絞り込みは書名・著者・ファイル名・サブフォルダを対象にする
- 同じ書名の本を見分けられるよう、棚の下のフォルダをカードに `📁 sub` として出す
  (棚直下の本には出さない)
- キーボードは `l` で開閉、`Escape` で閉じる。本棚を見ている間は
  `viewer-events.js` の `onKeyDown` を素通りさせる (隠れた iframe をページ送りしない)

**C3 の手動確認** (CLI の入口は C4 なので、棚を読み込んでサーバだけ起動する
使い捨てのハーネスを scratchpad に書いて確認した。**リポジトリには入れていない** —
C4-1 で入った `--library <フォルダ>` で同じことができるため、以後は不要):
本棚表示・絞り込み・4 通りのソート・本を開く・本の切り替え (同一 spine パス)・
インスペクタの追随・`l` / `Escape` / ✕・ライト / ダークの両テーマ・
再変換後の ⟳ で書名と表紙が更新されること (C2 の鮮度の面 3 つ) を実機で確認済み。

#### C3 以降への申し送り

- **状態を変えるエンドポイントは必ず POST にする** (前掲の不変条件)。
  本棚の再スキャンや削除を GET で足すと発信元検査を通らない
- ~~本棚 UI を足したら `viewer-library.js` を新設する~~ → **C3 で対応済**。
  以後もアセットを増やしたら `PreviewServer.ALLOWED_ASSETS` と `viewer.html` の
  script タグを**両方**更新すること (漏れは
  `PreviewServerTest.everyAssetReferencedByTheShellIsServable` が検出する)
- **📚 ボタンの有無は起動時の `api/session` 一回で決まる。** 起動後に棚が付く経路
  (稼働中のセッションに GUI から棚を足す・変換後の自動プレビューで棚を足す) では
  ボタンが出ない。→ **「棚は起動時に確定」で通すことに決定** (下の「C4 の設計判断」参照)
...
- **棚の再スキャン (フォルダに増えた本を拾う) はまだ無い。** C3 の ⟳ は
  既知の本を読み直すだけで、新しく増えたファイルは拾わない (ボタンの title に明記)。
  C4 で足すなら **POST** の `api/library/rescan` にし、`PreviewLauncher` に
  `LibraryIndexCache` を保持させる (現在はローカル変数で捨てている)

#### C4 の設計判断 (2026-08-09 確定。実装者 + Fable の見解一致)

**(1) 📚 ボタンの可視性は「棚は起動時に確定」で通す。**
C4 の 3 入口はいずれも最後に `PreviewLauncher.openInBrowser(url)` を呼び、
**セッションを再利用する場合でも新しいタブを開く**。新しいタブは必ず
`api/session` を取り直すので、実運用で困るのは「放置された古いタブに 📚 が出ない」
だけになる。古いタブを本当に生かすには棚の鮮度 (再スキャン / 監視) まで要り、
それは常駐設計と不可分なので、C4 で通知だけ先行させると
「ボタンは出るが棚は再スキャンされない」中途半端な状態を作ってしまう。

**将来 (Phase 3+) に「起動後に棚が付く」を成立させるときの設計** —
`PreviewSession` に単調増加の `revision` を持たせ (`setLibrary` / `addBook` /
更新で +1。書籍単位の `version` が既にあるので流儀は揃う)、heartbeat の応答を
204 から `{"revision":n}` の 200 に変える。ビューアーは起動時の値と比べ、
変わったときだけ `api/session` を 1 回取り直す (インメモリで stat 無し)。
`api/library` は本棚を開く操作でしか呼ばないままなので、
「全冊 stat をポーリングしない」制約を守れる。新しいポーリングも要らない。

**(2) GUI の「プレビュー」タブは C4 で新設する。**
C4 には「本棚を開く」ボタンと棚フォルダの選択という置き場所の要る UI が既にあるため、
初日から空のタブにならない。既存タブ 1 枚の構築コードは 50〜330 行
(Web タブ 50 行 / 文字タブ 120 行)、i18n はキー 1 個 × ja/en なのでタブ自体は安い。
既存の設定画面へ一時的に置くと Phase 3+ で引っ越すことになり、
ユーザーの操作記憶を壊す手戻りが確定する。
**未実装の設定欄 (常駐・固定ポート) は、グレーアウトで置かず、そもそも置かない。**

規模が気になる場合は「タブ新設だけの先行 PR (C4-0)」に切り出してよい
(C1〜C3 と同じ、小さく積む進め方)。

#### C4-1 の実装結果 (2026-08-09、PR #67)

計画どおり複数棚と CLI の入口が入った。**設計から変えた点が 1 つある**。

- **CLI のオプションは `--preview <フォルダ>` ではなく `--library <フォルダ>`** (複数指定可)。
  既存の `-preview` は引数を取らないフラグで、省略可能引数を持たせると
  `-preview book.epub` の EPUB を引数として食べてしまうため、別オプションにした。
  `--library` は `-preview` と同様、ブラウザを閉じるか Ctrl-C まで待機する
  (棚だけ読み込んで何も表示しないのでは意味がないため。ヘルプに明記)
- 棚は**ブラウザを開く前に**読み込む。ビューアーは起動時の `api/session` 一回で
  本棚ボタンを出すか決めるため、開いた後に足しても出ない
  (`PreviewLauncher.loadLibraryInto`)
- `loadLibraryInto` / `setLibrary` は棚を**足すのではなく差し替える**。
  棚を足していく運用が要るようになったら、まず session 側の契約を決めること

**レビューで見つかった、繰り返しやすい間違い** (C4-2 以降でも注意):

- 「棚を読み込んだか」を `errorCount` のような別の値で代用しない。
  1 冊目が見つからないだけで棚が一度も読み込まれず、本棚ボタンが永久に出なかった
- 早期 `return` で終了コードを捨てない。変換に失敗しても棚が開けたら 0 を返していた
- 「今このセッションを作ったか」は**処理の前に**決める。後から判定すると、
  既に開いている別のセッションを失敗時に巻き添えで落とす
- 上限は入れ子を畳み終わってから掛ける。途中で打ち切ると、
  後ろに来た親で畳めるはずの子が残ったまま数だけ埋まる

#### 本棚の複数フォルダ対応 (2026-08-09 ユーザー要望)

**実装状況**: 複数棚・重複排除・合計上限・INI キーの連番は C4-1 / C4-2a で入った。
この節のうち**未実装なのは次の 2 つ**で、着手ステージは未定 (常駐と同時期が自然):

- `POST /api/library/rescan` による明示の再スキャン (§「状態を変える口は POST」も参照)
- 初回スキャンの非同期化 (`PreviewLauncher.loadLibrary` は今も呼び出しスレッドで同期実行。
  GUI は C4-2a で専用スレッドから呼ぶことで UI の固まりを回避しているだけ)

棚を複数登録できるようにする。**C4 で API の形を決めるのが一番安い**
(まだリリースしていないため。`api/library` は現在 `folderName` 単数で、
本の位置は棚からの相対 `subFolder`)。

- **上限は「棚ごと」ではなく「合計」に掛ける。** `LibraryScanner.MAX_BOOKS = 2000` を
  棚ごとに適用すると 8 棚で 16000 冊になり、`api/library` を出すたびに
  16000 回 stat することになる (一覧は要求のたびに全冊 stat する設計)。
  効くコストは合計側なので上限も合計に掛ける。棚の数自体の上限は 8 程度でよい
- **絶対パスで重複排除する。** 「出力先フォルダ」と「その親」を両方登録するのは
  普通に起きる。入れ子の棚は同じ本を二重に数える
- **スキャンの時機**

  | 契機 | やること |
  |---|---|
  | 起動時 | 今と同じ 1 回。`LibraryIndexCache` が効けば ZIP を開かず stat だけ |
  | ユーザー操作 | 明示の再スキャン (`POST /api/library/rescan`、棚単位で指定可) |
  | 変換完了時 | スキャンし直さず、その 1 冊を `addBook` で足すだけ |
  | ファイル監視 | 常駐が入るまで見送り。短命プロセスでは割に合わない |

- **初回スキャンは非同期にする。** 初回だけ 1 冊ずつ ZIP を開いて OPF を読むので
  冊数に比例して重い (2 回目以降はキャッシュ命中で stat のみ)。
  現在の `PreviewLauncher.loadLibrary` は呼び出しスレッドで同期実行なので、
  棚を増やすならここを非同期にして UI を待たせないこと
- **INI キーは連番にする** (`PreviewLibraryDir.1` / `.2` …)。既存の `DstPathList` は
  カンマ区切りだが、Windows のフォルダ名にカンマを入れられるため、
  その弱点を新機能に持ち込まない
- **`api/library` の形**: `books[]` に所属棚を足し、トップに `shelves:[{name,count}]` を出す。
  ホスト上の絶対パスを載せない方針は維持する (棚は表示名だけ)。
  UI 側は棚での絞り込みを足す

#### Phase 2 で入れる小機能 (2026-08-08 ユーザー提案)

#### C4-2a の実装結果 (2026-08-09)

**着手前の未決事項はユーザーが決めた** (2026-08-09)。

- **棚フォルダの設定 UI は「リスト + 追加 / 削除ボタン」**。既存の `DstPathList` のような
  カンマ区切りのコンボボックスにはしない (フォルダ名にカンマを入れられるため)
- **C4-2 は 2 本の PR に分ける。** C4-2a = タブ新設 + 棚設定 + 「本棚を開く」、
  C4-2b = 変換後の自動プレビュー

実装で決めたこと・分かったこと:

- **棚の設定は全体設定 (`AozoraEpub3.ini`) に置き、`PreviewLibraryPrefs` に閉じ込めた。**
  `AozoraEpub3Applet` の `loadProperties` / `setProperties` は `profiles/*.ini` にも
  使われるため、そこへ混ぜるとプロファイルを切り替えるたびに棚が入れ替わる。
  読み書きを Swing から切り離したので、連番キーの挙動をユニットテストで押さえられる
  (`PreviewLibraryPrefsTest`、12 件)
- **書き戻す前に既存の連番キーを全部消す。** 上書きだけで済ませると、
  3 個から 2 個へ減らしたときに `PreviewLibraryDir.3` が残り、次の起動で棚が復活する
- **番号の抜けは詰めて読む** (手で編集した ini を黙って切り捨てない)。
  重複は正規化した絶対パスで畳むが、**大文字小文字は畳まない** —
  Windows では同じでも Linux では別のフォルダで、どちらに寄せても片方で間違えるため
- **終了時に設定が保存されていなかった** (master 由来の既存バグ)。
  26057a0 (JDK21 対応) で `finalize()` を `saveProperties()` にリネームした際、
  `windowClosing` の呼び出しが `applet.finalize()` (= `Object.finalize()`、何もしない)
  のまま残っていた。棚の永続化がこれに乗るため C4-2a で直した。
  副作用として**ウィンドウ位置・サイズ・出力先履歴なども再び保存されるようになる**
- 初回スキャンは重い (冊数に比例して ZIP を開く) ので、`PreviewLauncher.previewLibrary`
...
- **「処理中は押させない」をボタン 1 個の `setEnabled(false)` で表現しない。**
  一覧を選び直しただけで `updateLibraryButtons()` が走り、ボタンが戻って
  スキャンを二重に走らせられた。状態 (`libraryOpening`) を持ち、
  有効判定を 1 か所に集める (`isRunning()` と同じ扱い)
- **パスを `trim()` しない。** Unix 系ではフォルダ名の末尾に空白を置けるため、
  保存し直すたびに別のフォルダを指すようになる。空判定だけ `isBlank()` で行い、
  空でない値は 1 文字も削らない

`saveProperties()` を実際に呼ぶようにしたことで表面化した点 (レビュー: Opus):
...
- **数値の書式は `Locale.ROOT` で固定する。** 既定ロケールに任せると、
  小数点がカンマの環境 (de/fr) で `0.5` が `"0,5"` になり、
...
- `setConvertEnabled(true)` は変換 worker の `doInBackground` の finally からも呼ばれ、
  **EDT 外で Swing コンポーネントを触っている** (既存の穴)。
  `updateLibraryButtons()` は自分で EDT へ移して身を守っているが、根治はしていない
- `openPreview()` には `libraryOpening` に相当するフラグが無く、起動中に変換が終わると
  `updatePreviewButton()` でボタンが戻って二重起動できる。**C4-2b で揃える**
...
### Phase 3+ 候補 — 常駐サーバと「プレビュー」タブ (2026-08-09 ユーザー提案)

C3 で本棚が入り、「変換結果の確認」から「PC 用ビューアー」へ用途が寄ってきたため、
**サーバを常駐させたい**という要望が出た。ただし常駐は寿命・セキュリティ・
設定の 3 面で設計が別物になるので、**先に効く 2 つを済ませてから**着手する。

**先に入れると常駐しなくても実用になるもの (推奨順)**

1. **読書位置の保存** (Phase 4 の「しおり / 読書位置の保持」と同じ項目)。
   タブを開き直せば続きから読めるなら、常駐の必要性はかなり下がる
2. **安定 URL** — 固定ポート + トークンの永続化。現状はポートもトークンも
   起動のたびに変わるので**ブックマークできない**。常用するならここが本丸

**常駐そのものに付いてくるもの**

| 項目 | 中身 |
|---|---|
| 設定 | **アプリで常駐する / しないを設定できること** (ユーザー要望)。既定は OFF |
| 起動導線 | トレイ常駐 (Swing `SystemTray`) と多重起動防止。ポートの奪い合いを避ける |
| 棚の鮮度 | 監視 (`WatchService`) か定期再スキャン。「起動時 1 回スキャン」の前提が崩れる |
| 展開キャッシュ | 生き続けるなら LRU と容量上限が要る (今はセッション終了で消える一時ディレクトリ) |
| セキュリティ | トークンを永続化すると URL がブラウザ履歴・同期に残る。漏れれば棚全体の閲覧権になる。<br>今は「毎回使い捨て」が効いているので、永続化するなら失効・再生成の口が要る |

**「プレビュー」タブへ足す設定** (タブ自体は C4-2a で新設済み)

タブは 9 枚目として既にある (`ui.tab.preview`、棚のフォルダと「本棚を開く」を収めた)。
**ここで新設を計画し直さないこと。**常駐が入るときに足すのは次の設定だけ:

- 常駐する / しない・固定ポート・再スキャンの間隔・展開キャッシュの上限
- 変換後の自動プレビュー (`AutoPreview`) は C4-2b で同じタブに入る
- 文言は `ui.tab.*` と同様 **ja / en の両方**に足す
  (`src/i18n/messages_ja.properties` / `messages_en.properties`。`feedback_docs_sync` と同様、
  片方だけ足さない)
- 設定の保存先は既存に合わせて `AozoraEpub3.ini` (`AozoraEpub3Applet.props`)。
  棚と同じく**全体設定に置く** (プロファイルには混ぜない)

### Phase 4 以降 (未着手・要検討)

- セピアテーマ
- `.kepub.epub` の挙動確認
- しおり / 読書位置の保持 (上記のとおり、常駐より先に入れる価値がある)
...
**設計上の論点 — バッチ変換時のサーバー/ポートの扱い (2026-08-11 調査)**:
- 現行実装に**プロセスをまたぐ引き継ぎ機構はない**。ポートは
  `PreviewServer.java:136` の `new InetSocketAddress(loopback, 0)` で毎回 OS 任せ
  (衝突しない代わりに毎回変わる)。`PreviewLauncher.preview()` の
  「既存サーバーへ本を追加」(`current` static) は**同一 JVM 内限定**
- よって案 A × narou.rb バッチ N 冊 = 子プロセス N 個・サーバー N 個・タブ N 枚。
  プロセスとポートは heartbeat (タブを閉じたら自滅) で自動回収されるため実害は
  タブ枚数のみ
- バックグラウンドタブの heartbeat 間引き対策 (タブ単位の生存管理 + 5 分猶予 +
  `visibilitychange`/`pageshow`/`focus` での即時送信、本書 §終了条件参照) は
  PreviewServer 側の実装なので、**spawn した子プロセスにも追加実装なしで乗る**。
  タブが背面に積まれても誤検知で子プロセスが死ぬことはない
- **v1.5.1 は「毎回新規」で割り切る (2026-08-11 方針)**。既存サーバーへの
  引き継ぎ型は (1) ポート + 認証トークンを書いた発見ファイル、(2) 外部から
  `session.addBook` を呼ぶ HTTP API、(3) localhost の他プロセス/悪意ページ対策の
  トークン必須化、が必要でパッチの粒度を超える。発見ファイル不在時は新規起動に
  フォールバックできるため、後付けしても互換性は壊れない — **v1.6 改良候補**
