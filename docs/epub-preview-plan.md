...
#### 本棚の複数フォルダ対応 (2026-08-09 ユーザー要望。C4-1 で実装済)
...
**GUI に「プレビュー」タブを新設する** (ユーザー要望)

現在の `JTabbedPane` は 変換 / 画像1 / 画像2 / 詳細設定 / 目次 / スタイル / 文字 / Web の
8 枚 (`AozoraEpub3Applet.java:772-2261`、ラベルは `ui.tab.*`)。ここに 9 枚目として
プレビュー関連をまとめる。**タブを足すと各設定の置き場所が決まるので、
C4 の自動プレビュー設定より先にタブの有無を決めた方が手戻りが無い。**

- 集約する設定: 変換後の自動プレビュー (C4 の `AutoPreview`)・常駐する/しない・
  固定ポート・棚のフォルダ・再スキャンの間隔・展開キャッシュの上限
- ラベルは `ui.tab.preview` を **ja / en の両方**に追加する
