...
| C4-2 | GUI: 「プレビュー」タブ新設 + 「本棚を開く」+ 変換後の自動プレビュー (下記 (a)) | 未着手 |
...
- **棚フォルダの設定 UI の形。** 最大 8 個をどう見せるか。
  実装者の案は**リスト + 追加 / 削除ボタン** (既存の `DstPathList` のような
  カンマ区切りのコンボボックスにはしない。フォルダ名にカンマを入れられるため)。
  **ユーザーの確認待ち (2026-08-09 時点)**
- INI キーは連番 (`PreviewLibraryDir.1` / `.2` …)。上記と同じ理由
- 初回スキャンは重い (冊数に比例して ZIP を開く) ので、
  `PreviewLauncher.loadLibrary` を EDT から直接呼ばず `SwingWorker` に載せる。
