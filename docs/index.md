...
- **Java 26 推奨**（最新 GA）
  - Java 21 LTS / Java 25 LTS との互換性を維持しています
  - 最低動作要件は **Java 21 以降**
- Windows / macOS / Linux

Java をお持ちでない場合は、[Eclipse Temurin](https://adoptium.net/temurin/releases/) から Java 26 をダウンロードしてください（LTS が必要なら Java 21 でも動作します）。

---

## Java 26 のインストール（Eclipse Temurin）

### Windows

1. [Adoptium Releases](https://adoptium.net/temurin/releases/) を開きます
2. JDK 26 → Windows x64 → `.MSI` を選択しダウンロードします
...
1. [Adoptium Releases](https://adoptium.net/temurin/releases/) を開きます
2. JDK 26 → macOS → `.PKG`（Intel または Apple Silicon M1/M2）をダウンロードします
...
1. [Adoptium Releases](https://adoptium.net/temurin/releases/) を開きます
2. JDK 26 → Linux x64 → `.TAR.GZ` をダウンロードします
3. 展開します: `tar -xzf OpenJDK26U-jdk_x64_linux_hotspot_26_x.tar.gz`
4. PATH に追加するか、`./jdk-26.x.x+yy/bin/java -version` で確認します
...
- **Java がインストールされていない場合** — [Temurin](https://adoptium.net/temurin/releases/) から Java 26 をダウンロードしインストールしてください（Java 21 以降であれば動作します）。
