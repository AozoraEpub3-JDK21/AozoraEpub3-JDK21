...
| バージョン | 内容 |
|---|---|
| ~~v1.3.7-jdk21~~ | **本計画のステージは未着手のままリリース済み**（2026-07-25）。コード監査 #1〜#17 のバグ修正リリースになった |
| v1.3.8-jdk21 | ステージ A (PR #1) — 電書連 v1.1.4 自己宣言可 |
| v1.3.9-jdk21 | ステージ B (PR #2) — CSS バグ修正 |
| v1.4.0-jdk21 | ステージ C (PR #3) — `dpfjCompliantMode` フラグ追加（デフォルト false） |
| v2.0.0-jdk21 (将来) | `dpfjCompliantMode=true` デフォルト化を検討 |

> 当初はステージ A を v1.3.7 に載せる計画だったが、コード監査キャンペーン（#1〜#17）の
> 修正が先に積み上がったため、v1.3.7 はバグ修正リリースとして先行させた。
> ステージ A 以降は 1 つずつ後ろにずらす。
