...
### ✅ AozoraTextFinalizer による後処理（7項目 → 全て実装済み）

以下の項目は `AozoraTextFinalizer.java` によるファイナライズ処理で実装済み。
TextNode レベルの処理（printText内）と、ファイル全体を対象とした後処理の2段階で対応。

#### 11. かぎ括弧内の自動連結（3.5） ✅ 完了
**実装日**: 2026-02-15
**実装方式**: 2段階処理
- `WebAozoraConverter.autoJoinInBrackets()` - TextNode内の改行処理
- `AozoraTextFinalizer.autoJoinInBrackets()` - `<br>`タグ由来の改行にも対応（ファイル全体の後処理）

---

#### 12. 行末読点での自動連結（3.6） ✅ 完了
**実装日**: 2026-02-15
**実装方式**: 2段階処理
- `WebAozoraConverter.autoJoinLine()` - TextNode内の読点連結
- `AozoraTextFinalizer.autoJoinLine()` - `<br>`タグ由来の改行にも対応（ファイル全体の後処理）

---

#### 13. 前書き・後書きの自動検出（3.2） ✅ 完了
**実装日**: 2026-02-15
**実装メソッド**: `AozoraTextFinalizer.detectAndMarkAuthorComments()`
- `*`が44個の行 → `［＃ここから前書き］` / `［＃ここで前書き終わり］`
- `*`が48個の行 → `［＃ここから後書き］` / `［＃ここで後書き終わり］`

---

#### 14. 自動行頭字下げ（3.4） ✅ 完了
**実装日**: 2026-02-15
**実装メソッド**: `AozoraTextFinalizer.applyAutoIndent()`
- 段落開始（前の行が空行）を検出して全角スペースで字下げ
- 既に字下げ済み、注記行、かぎ括弧、見出し等はスキップ

---

#### 15. 改ページ直後の見出し化（3.12） ✅ 完了
**実装日**: 2026-02-15
**実装メソッド**: `AozoraTextFinalizer.enchantMidashi()`
- `［＃改ページ］`直後の行を `［＃３字下げ］［＃中見出し］...［＃中見出し終わり］` で囲む

---

#### 16. かぎ括弧の開閉チェック（3.13） ✅ 完了
**実装日**: 2026-02-15
**実装メソッド**: `AozoraTextFinalizer.inspectBrackets()`
- 行単位でかぎ括弧のネスト深度をチェック、不整合時にログ警告

---

#### 17. replace.txt の適用（4.2） ✅ 完了
**実装日**: 2026-02-15
**実装メソッド**: `AozoraTextFinalizer.applyReplacePatterns()`
- `replace_narourb.txt` からタブ区切りの置換ルールを読み込み、テキスト全体に適用
- `NarouFormatSettings.loadReplacePatterns()` でファイル読み込み
...
### ✅ 採用: 案1（設定ダイアログの追加） — 実装完了

**実装ファイル**:
- `src/com/github/hmdev/swing/NarouFormatSettingsDialog.java` — 設定ダイアログ
- `src/com/github/hmdev/web/NarouFormatSettings.java` — `save()` メソッド追加
- `src/AozoraEpub3Applet.java` — 「詳細設定を開く...」ボタン追加

**ダイアログ構成** (3セクション):
1. **テキスト変換** — 二分アキ、漢数字化、記号全角化、分数/日付変換、三点リーダー等
2. **ファイナライズ処理** — 前書き/後書き検出、字下げ、見出し化、括弧チェック
3. **表示・メタデータ** — あらすじ、URL、更新日時、終了マーカー

---

## 📝 narou.rb 3.9.1 との差異修正（2026-02-15 実施）

### 修正済みの差異

| 項目 | 修正前 | 修正後 (narou.rb互換) |
|------|--------|----------------------|
| ローマ数字変換 | Ⅱ→II (Unicode→alphabet) | II→Ⅱ (alphabet→Unicode) |
| 英文判定 | 8文字以上+正規表現 | 2単語以上 or 一定長+英字含む |
| 感嘆符後の全角アキ | なし | `！あ` → `！　あ` (閉じ括弧前は除外) |
| 小説ルール変換 | なし | `。」`→`」`、省略記号の偶数化 |
...
全ての変換処理は以下の2層で実装済み:
- **printText() 内パイプライン**: TextNode単位のリアルタイム変換
- **AozoraTextFinalizer**: ファイル全体を対象とした後処理
- **NarouFormatSettingsDialog**: GUI設定ダイアログ
...
|---------|------|---------|
| GUI設定ダイアログ | ✅ 完了 | — |
| narou.rb 3.9.1 差異修正 | ✅ 完了 | — |
| narou.rb出力比較テスト | 実機テスト未実施 | 2-4時間 |

---

## 🚀 今後の実装ロードマップ

### Step 1: GUI設定ダイアログの追加
- `NarouFormatSettingsDialog.java` の作成
- `AozoraEpub3Applet.java` へのメニュー項目追加
- 設定の保存処理（`save()` メソッド追加）
...
**Phase 1**: ✅ 完了 (3/3)
**Phase 2**: ✅ 完了 (3/3)
**Phase 3**: ✅ 完了 (14/14) — AozoraTextFinalizer により全項目対応
**Phase 4**: ✅ 完了 (2/2) — 設定読み込み + replace.txt 適用
**GUI設定ダイアログ**: ✅ 完了 (NarouFormatSettingsDialog + save())
**narou.rb 3.9.1差異修正**: ✅ 完了 (ローマ数字、英文判定、全角アキ、小説ルール)
**narou.rb出力比較テスト**: 実機テスト未実施
