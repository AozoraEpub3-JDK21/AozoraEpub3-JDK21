...
## 未リリース（v1.5.0-jdk21 予定）

> リリース時はこの節を「## バージョン: 1.5.0-jdk21」+ リリース日に書き換える。
...
- **GUI の見た目を FlatLaf でモダン化**。ライト / ダークテーマを同梱し、既定はライト。
...
- **カクヨムで新着話を取り逃していた不具合を修正**（監査項目 21）。
...
- **目次以外も、ini にキーが無いときの CLI 既定値が GUI に揃った**（項目 24）。
  ini を渡さない場合やキーの少ない自作 ini では、表題ページが出る（従来はなし）・
  濁点は合成フォント方式になる（`DakutenType` 0 → 1）・自動改ページが効く・
  大きい挿絵が画面サイズに合わせて縮小される・表紙のタイトル行数が 10 行で切られる
  （従来は実質無制限）など**出力が変わる**。実測では ini なしの 1.8MB 見出し無しテキストが
  本文 XHTML 1 枚 → 5 枚 + 表題ページになった。GUI が保存した全キーの ini と
  narou.rb 連携先の実 ini（123 キー）では `dcterms:modified` 以外の差分ゼロを確認済み
- **同梱 `AozoraEpub3.ini` を 24 キーから全キー版に拡充した**。値は「新規展開した GUI が
  v1.4.x と同じ状態で起動する」ように、旧 ini にあったキーはそのまま・無かったキーは
  GUI のウィジェット初期値。このため**同梱 ini のまま CLI 変換していた場合は、
  目次ページが縦書きになる・自動改ページが効く・画像の画面サイズ調整が有効になる・
  JPEG 品質が 80 → 85 になるなど出力が変わる**（GUI で一度でも設定を保存した環境は
  ini が全キーで上書き済みのため不変）。従来の CLI 出力に固定したい場合は
  `test_data/reference_comparison.ini` 末尾の 8 キーと同じ値を ini に明示する
- **CLI が jar と同じ場所の ini も読むようになった**（項目 26）。探索順は
...
- **`GothicUseBold` が効くようになった**（項目 25 の一部）。CLI 側のキー名タイポ
...
- #72 で章が復活する作品では、既存の未対応挙動（章タイトルページの `［＃ここから柱］` が
  ...
  見えるようになる
- `.NET` ポート `JavaComparisonTests` 5/5 PASS を維持（出力 EPUB の構造は不変）
