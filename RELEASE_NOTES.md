...
- **端末で表示できない外字を注記表示にフォールバックする設定を追加**（新機能・**既定は無効**）。
  ...
  - **仮名＋濁点/半濁点は 2 文字で 1 字として扱う**。`※［＃半濁点付き平仮名か、1-4-87］` のように
    2 文字（`か` ＋ 結合半濁点）に変換される注記は、同梱の濁点フォント（`DakutenType=2`）や
    CSS 合成で表示できるので、対で出せるなら抑止しない。出せない場合は片方だけでなく
    **対ごと**注記表示にする（`〓` と濁点が分かれて `〓゛` になることはない）

**設定を変更しない限り、出力される EPUB は従来と 1 バイトも変わらない。**
`DakutenType=2`（配布既定）と `DakutenType=1` の両方で、修正前後の EPUB が
内容 byte-identical であることを確認している。

### ドキュメント

- 「外字の設定」ページを日英で新規追加（`docs/gaiji-settings.md` / `docs/en/gaiji-settings.md`）。
  GUI のスクリーンショット付きで、水準の選び方と 1 文字フォントの置き方を説明している
  （スクリーンショットは背景の写り込みを落として枠ぴったりに crop し直した）
...
- レビューで見つかった判定漏れを修正（いずれも「水準判定より手前で文字を出力していた」もの）
  - 注記経由の外字で 1 文字フォントの免除が効いていなかった
  - 異体字セレクタ付きの文字が判定を素通りしていた（`convertTcyText` の IVS/VS 4 分岐）
  - 本文とルビの長さが同じとき（`俠《き》`）に判定を素通りしていた
  - **同梱の濁点フォントで表示できる文字まで `〓` にしていた**。濁点フォントは
    `utf16FontMap` 等のマップではなく `gaiji/dakuten/u<base>-u<mark>.ttf` を
    ファイル名で直接引く別経路なので、マップ照会では引っかからなかった
- 濁点付き文字の判定を `isDakutenPairFallback` に集約し、**経路ごとに「実際に何を出すか」で
  判断する**ようにした。合成済み 1 文字になるなら合成後の水準、縦書き＋`DakutenType=2` で
  同梱フォントがあれば免除、それ以外は基底字の水準で決める。
  基底字を `〓` に落としたときは直後の濁点も落とす（`〓` を出す 8 経路すべて）
- 結合濁点/半濁点 U+3099/U+309A を、異体字セレクタと同じく水準判定の対象外にした。
  単体では字にならず、出力時に U+309B/U+309C（第1・2水準）に正規化されるため
...
- フレッシュコンテキストのサブエージェント（Codex / Fable / Opus）によるレビューを、
  指摘ゼロになるまで反復（Codex 5 周・Fable 2 周・Opus 3 周）
